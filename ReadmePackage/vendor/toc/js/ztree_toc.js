...
; (function ($) {
	...
	function bind_scroll_event_and_update_postion(opts) {
		...
		if (opts.highlight_on_scroll) {
			$('.markdown-body').bind('scroll', highlight_on_scroll);
			highlight_on_scroll();
		}
	}
	...
	$.fn.ztree_toc.defaults = {
		...
		_headers: [],
		_header_offsets: [],
		_header_nodes: [{ id: 1, pId: 0, name: 'DZSuperFrame中文文档', open: true }],
		debug: true,
		highlight_offset: 0,
		...
		ztreeSetting: {
			...
			callback: {
				beforeExpand: function (treeId, treeNode) {
					setTimeout(() => {
						$('ul').each(function () { if ($(this).css('overflow') === 'hidden') { $(this).css('overflow', '') } });
					}, 300);
				},
